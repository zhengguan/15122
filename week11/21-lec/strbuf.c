...
void strbuf_delete(struct strbuf *sb, size_t i);
char strbuf_pop(struct strbuf *sb);
void strbuf_push(struct strbuf *sb, char c);
...
// WHERE: (0 <= i) && (i < sb->len);
// EFFECT:remove the ith char of sb
void strbuf_delete(struct strbuf *sb, size_t i) {
  REQUIRES(is_strbuf(sb));
  REQUIRES(i < sb->len);
...
// WHERE: sb->len > 0;
// EFFECT: pop the last char of sb
// RETURNS: the last char of sb
char strbuf_pop(struct strbuf *sb) {
  REQUIRES(is_strbuf(sb));
  REQUIRES(sb->len > 0);

  char c = sb->buf[sb->len - 1];
  strbuf_delete(sb, sb->len - 1);
